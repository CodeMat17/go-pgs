...
const PhdFeesContainer = () => {
  const genFees = useQuery(api.fees.getPhdGeneralFees);
    ...
    const eduPlusFees = useQuery(api.fees.getPhdEduFees)
    ...
    <div>
      ...
      {genFees === undefined ? (
        ...
        <div className='pb-8'>
          ...
          {/* Faculty of Nat Sciences et el */}
          {natSciFees === undefined ? (
            ""
          ) : (
            <div className='bg-white dark:bg-gray-900 rounded-xl p-5 shadow-md mb-4'>
          ...
          {/* Faculty od Education et el */}
          {eduPlusFees === undefined ? (
            ""
          ) : (
            <div className='bg-white dark:bg-gray-900 rounded-xl p-5 shadow-md'>
              <h1 className='text-muted-foreground font-semibold mb-4'>
                Faculty of Education/Arts/Management Sciences (PhD)
              </h1>
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                {eduPlusFees.map((fee) => (
                  <div key={fee._id} className=''>
